#!/usr/bin/env python3
"""
Copy Docify hub skills into ``.cursor/skills/`` for Cursor project skill
discovery.

Canonical sources:
``'fy'-suites/docify/superpowers/<skill-name>/SKILL.md`` Targets:
.cursor/skills/<skill-name>/SKILL.md

Run from repository root after editing any skill under
``'fy'-suites/docify/superpowers/``:
...
def sync(*, check_only: bool) -> int:
    """Copy Docify router skills to ``.cursor/skills`` or verify they
    match.
